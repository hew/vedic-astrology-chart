...
import { calculateHouses, getPlanetDisplay, getPlanetDegree } from '../../utils';
import { formatAscendantDegree, getAscendantSign, getPlanetColor } from './utils/helpers';

interface ChartProps extends Omit<VedicChartProps, 'style'> {}
...
const NorthIndianChart: React.FC<ChartProps> = ({
  ...
  const houseNumberPositions = [
    // 1st house 
    { num: 1, x: centerX, y: centerY - size * 0.02, label: getHouseSignNumber(1).toString() },
    
    // 2nd house
    { num: 2, x: centerX - size * 0.2, y: centerY - size * 0.22, label: getHouseSignNumber(2).toString() },
    
    // 3rd house
    { num: 3, x: centerX - size * 0.23, y: centerY - size * 0.192, label: getHouseSignNumber(3).toString() },
    
    // 4th house 
    { num: 4, x: centerX - size * 0.03, y: centerY + size * 0.008, label: getHouseSignNumber(4).toString() },
    
    // 5th house
    { num: 5, x: centerX - size * 0.23, y: centerY + size * 0.21, label: getHouseSignNumber(5).toString() },
    
    // 6th house 
    { num: 6, x: centerX - size * 0.2, y: centerY + size * 0.24, label: getHouseSignNumber(6).toString() },
    
    // 7th house 
    { num: 7, x: centerX, y: centerY + size * 0.04, label: getHouseSignNumber(7).toString() },
    
    // 8th house 
    { num: 8, x: centerX + size * 0.2, y: centerY + size * 0.24, label: getHouseSignNumber(8).toString() },
    
    // 9th house 
    { num: 9, x: centerX + size * 0.23, y: centerY + size * 0.21, label: getHouseSignNumber(9).toString() },
    
    // 10th house 
    { num: 10, x: centerX + size * 0.03, y: centerY + size * 0.008, label: getHouseSignNumber(10).toString() },
    
    // 11th house 
    { num: 11, x: centerX + size * 0.23, y: centerY - size * 0.192, label: getHouseSignNumber(11).toString() },
    
    // 12th house 
    { num: 12, x: centerX + size * 0.2, y: centerY - size * 0.22, label: getHouseSignNumber(12).toString() }
  ];
  ...
  return (
    <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`}>
      ...
      {showHouseLabels && houseNumberPositions.map(({ num, x, y, label }) => (
        <text
          key={`house-${num}`}
          x={x}
          y={y}
                ...
                <g key={planetName}>
                  ...
                  >
                    ...
                    <tspan fontWeight="normal" fill="#666">
                      {' '}{house.sign.substring(0, 3)} {getPlanetDegree(planetName, planets)}
                    </tspan>
                  </text>
                </g>
              );
